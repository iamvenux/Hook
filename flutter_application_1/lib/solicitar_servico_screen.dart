...
// ─────────────────────────────────────────────
// Configuração do serviço — define TUDO que muda
// entre um pedido de reboque e um pedido de ajuda.
// Adicionar um novo tipo de serviço = adicionar uma
// entrada num dos mapas abaixo, sem tocar nas telas.
// ─────────────────────────────────────────────
...
class ServicoConfig {
  ...
  String get tituloResumo => isReboque ? 'RESUMO DO SERVIÇO' : 'SOLICITAR AJUDA';
  String get labelBotaoResumo => isReboque ? 'Solicitar guincho' : 'Solicitar Ajuda';
  String get labelProfissional => isReboque ? 'guincho' : 'socorrista';
}
...
// ─────────────────────────────────────────────
// Tela 1 — Resumo do Serviço (reboque OU ajuda)
// ─────────────────────────────────────────────
...
class _ResumoServicoScreenState extends State<ResumoServicoScreen> {
  ...
  Future<void> _confirmarSolicitacao() async {
    if (_servico.exigeDescricao && _descricaoController.text.trim().isEmpty) {
    ...
    try {
      final solicitacao = await ApiService.instance.criarSolicitacao(
        veiculoId: widget.veiculoId,
        tipoReboque: _servico.titulo, // 'Guincho Leve' ou 'Guincho Pesado'
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (_) => ProcurandoProfissionalScreen(
            servico: _servico,
            endereco: _endereco,
            ...
            child: Container(
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    Text(_servico.tituloResumo,
                        style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: pretoPrincipal, letterSpacing: 0.3)),
                    const SizedBox(height: 16),

                    // Card do serviço escolhido (tipo de reboque OU tipo de ajuda)
                    Container(
                      ...
                      child: Row(
                        children: [
                          Container(
                            width: 46, height: 46,
                            decoration: BoxDecoration(color: azulPrincipal.withOpacity(0.12), borderRadius: BorderRadius.circular(12)),
                            child: Icon(_servico.icone, color: azulPrincipal, size: 24),
                          ),
                          ...
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(_servico.titulo,
                                    style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: pretoPrincipal)),
                                const SizedBox(height: 2),
                                Text(_servico.subtitulo, style: const TextStyle(fontSize: 12, color: cinzaTexto)),
                              ],
                            ),
                          ),
                        ],
                      ...
                      child: Row(
                        children: [
                          Container(
                            ...
                            child: const Icon(Icons.directions_car_rounded, color: pretoPrincipal, size: 22),
                          ),
                          const SizedBox(width: 12),
                          Expanded(
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              const Text('VEÍCULO', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: cinzaTexto, letterSpacing: 0.8)),
                              const SizedBox(height: 3),
                              Text(widget.veiculo, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: pretoPrincipal)),
                            ]),
                          ),
                        ],
                      ),
                    ...
                    // Descrição do problema — só quando o serviço exige (ex.: "Outros")
                    if (_servico.exigeDescricao) ...[
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(
                            color: _tentouEnviar && _descricaoController.text.trim().isEmpty
                                ? Colors.red
                                : Colors.transparent,
                            width: 1.4,
                          ),
                        ...
                        child: TextField(
                          ...
                          maxLines: 3,
                          minLines: 2,
                          style: const TextStyle(fontSize: 14, color: pretoPrincipal),
                          decoration: const InputDecoration(
                            hintText: 'Descreva o problema com o veículo...',
                            hintStyle: TextStyle(color: cinzaTexto, fontSize: 14),
                            border: InputBorder.none,
                            contentPadding: EdgeInsets.symmetric(vertical: 14),
                          ),
                        ),
                      ),
                      if (_tentouEnviar && _descricaoController.text.trim().isEmpty)
                        const Padding(
                          padding: EdgeInsets.only(top: 6, left: 4),
                          child: Text('Conte pra gente o que está acontecendo.',
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          FittedBox(
                            ...
                            child: Text(_servico.valorEstimado,
                      ...
                      child: Padding(
                        ...
                        child: SizedBox(
                          ...
                          child: ElevatedButton(
                            onPressed: _enviando ? null : _confirmarSolicitacao,
                            style: ElevatedButton.styleFrom(
                              backgroundColor: pretoPrincipal,
                              foregroundColor: Colors.white,
                              disabledBackgroundColor: pretoPrincipal.withOpacity(0.6),
                            ...
                            child: _enviando
                                ...
                                : Text(_servico.labelBotaoResumo,
...
// ─────────────────────────────────────────────
// Tela 2 — Procurando / Acompanhando Profissional
// (guincheiro OU socorrista, conforme servico.categoria)
// ─────────────────────────────────────────────
class ProcurandoProfissionalScreen extends StatefulWidget {
  final ServicoConfig servico;
  final String endereco;
  final LatLng coordenada;
  final String veiculo;
...
class _ProcurandoProfissionalScreenState
  ...
  // Estado vindo da API a cada consulta.
  String  _status    = 'buscando';
  String  _mensagem  = 'Procurando o guincho mais próximo...';
  ...
  void initState() {
    ...
    _consultarStatus(); // primeira consulta imediata
    _pollingTimer = Timer.periodic(const Duration(seconds: 4), (_) => _consultarStatus());
  }
  ...
  Future<void> _consultarStatus() async {
    try {
      final resp = await ApiService.instance.consultarSolicitacao(widget.solicitacaoId);
      if (!mounted) return;

      final solicitacao = resp['solicitacao'] as Map<String, dynamic>;
      final novoStatus  = solicitacao['status'] as String;
      final motorista   = resp['motorista'] as Map<String, dynamic>?;

      setState(() {
        _status     = novoStatus;
        _mensagem   = (resp['mensagem'] as String?) ?? _mensagem;
        _motorista  = motorista;
        _carregando = false;
        _erro       = false;
      });

      // Painel sobe quando já tem motorista atribuído.
      if (motorista != null && _sheetController.size < _tamanhoAceitePainel - 0.02) {
      ...
      // Se o motorista já tem localização, desenha o marcador + rota
      // e para de dar zoom automático toda hora.
      if (motorista != null &&
          motorista['latitude'] != null &&
          motorista['longitude'] != null) {
        final posMotorista = LatLng(
          double.parse(motorista['latitude'].toString()),
          double.parse(motorista['longitude'].toString()),
        );
        ...
        setState(() {
          _markers = {
            ...
            Marker(
              ...
              infoWindow: InfoWindow(title: motorista['nome'] as String?),
            ),
          };
        });

        _buscarRota(posMotorista, widget.coordenada);
      }

      // Para o polling quando a corrida termina (concluída/cancelada).
  ...
  Future<void> _buscarRota(LatLng origem, LatLng destino) async {
    ...
    try {
      ...
      setState(() {
        _polylines = {
          Polyline(
            polylineId: const PolylineId('rota_profissional'),
            points: pontos,
            color: azulPrincipal,
            width: 5,
          ),
      ...
      // Sem internet ou OSRM fora do ar — a tela continua funcionando,
      // só sem desenhar a rota.
    }
  }
  ...
  Future<void> _cancelar() async {
    try {
      await ApiService.instance.cancelarSolicitacao(widget.solicitacaoId);
    } catch (_) {
      // mesmo se a chamada falhar, deixa o usuário sair da tela
    }
  ...
  Widget build(BuildContext context) {
    ...
    final temMotorista        = _motorista != null;
          ...
          Positioned(
            ...
            child: Container(
              ...
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(_servico.icone, color: azulPrincipal, size: 16),
                  const SizedBox(width: 6),
                  Text(_servico.titulo,
                      style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: pretoPrincipal)),
                ],
              ),
            ),
          ),
              ...
              return Container(
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    if (_erro)
                      const Text('Não foi possível consultar o status agora. Tentando de novo...',
                          style: TextStyle(fontSize: 14, color: Colors.red))
                    else
                      Text(_mensagem,
                          style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: pretoPrincipal)),
                    ...
                    if (!temMotorista) ...[
                      ...
                      Container(
                        ...
                        child: Row(
                          children: [
                            const Icon(Icons.location_on_rounded, color: pretoPrincipal, size: 16),
                            const SizedBox(width: 6),
                            Expanded(
                              child: Text(widget.endereco,
                                  style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: pretoPrincipal),
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(_motorista!['nome'] as String? ?? 'Motorista',
                                      style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white)),
                                  const SizedBox(height: 6),
                                  if (_motorista!['placa'] != null)
                                    Container(
                                      ...
                                      child: Text(_motorista!['placa'] as String,
                                          style: const TextStyle(fontSize: 12, color: Colors.white)),
                                    ),
                                ],
                              ),
                            ),
                            Text(_servico.valorEstimado,
                                style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: Colors.white)),
                          ],
                        ),
                      ),
                      const SizedBox(height: 16),
                      if (_motorista!['telefone'] != null)
                        SizedBox(
                          width: double.infinity,
                          child: ElevatedButton.icon(
                            icon: const Icon(Icons.phone_outlined, size: 18),
                            label: Text('Ligar para ${_motorista!['telefone']}'),
                            style: ElevatedButton.styleFrom(
                      ...
                      SizedBox(
                        ...
                        child: OutlinedButton(
                          ...
                          style: OutlinedButton.styleFrom(
                            foregroundColor: pretoPrincipal,
                      ...
                      Container(
                        ...
                        child: const Row(
                          children: [
                            ...
                            SizedBox(width: 10),
                            Expanded(child: Text('Serviço concluído! Obrigado por usar o Hook.')),
                          ],
                        ),
                      ),
